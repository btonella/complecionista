...
class _PodcastPageState extends State<PodcastPage> {
  ...
  void initState() {
    super.initState();
    feedCubit.getEspecificRSSEvent('podcast');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: bottomAppBar(context, 'podcasts'),
      ...
      body: BlocConsumer<FeedCubit, FeedState>(
        ...
        listener: (context, state) {
          ...
          } else if (state is FeedSuccessState && state.data.containsKey('podcast')) {
            setState(() {
              feed = state.data['podcast'];
            });
          }
        },
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            if (index == 0) {
                              ...
                              List temp = item.categories != null
                                  ? item.categories!.map(
                                      (e) {
                                        if (!e.value.contains('Notícias') && !e.value.contains('Games') && !e.value.contains('Podcast')) {
                                          return Container(
