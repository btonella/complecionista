...
class _VideosPageState extends State<VideosPage> {
  ...
  void initState() {
    super.initState();
    feedCubit.getEspecificRSSEvent('videos');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: bottomAppBar(context, 'videos'),
      ...
      body: BlocConsumer<FeedCubit, FeedState>(
        ...
        listener: (context, state) {
          ...
          } else if (state is FeedSuccessState && state.data.containsKey('videos')) {
            setState(() {
              feed = state.data['videos'];
            });
          }
        },
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            if (index == 0) {
                              ...
                              List temp = item.categories != null
                                  ? item.categories!.map(
                                      (e) {
                                        if (!e.value.contains('Notícias') && !e.value.contains('Games') && !e.value.contains('Vídeos')) {
                                          return Container(
